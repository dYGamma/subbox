...
install-claude:
	...
	install -Dm755 integrations/claude-code/proxy-ensure.sh \
		"$(HOOK_DIR)/proxy-ensure.sh"
	@echo
...
uninstall:
	...
	rm -rf "$(SHAREDIR)"
	rm -f "$(CLAUDE_BIN)/claude" "$(HOOK_DIR)/proxy-ensure.sh"
...
lint:
	...
	shellcheck install.sh integrations/claude-code/proxy-ensure.sh
...
**Files:**
- Create: `integrations/claude-code/claude`, `integrations/claude-code/proxy-ensure.sh`, `integrations/claude-code/README.md`, `tests/test_claude_wrapper.py`
...
HOOK = Path(__file__).resolve().parents[1] / "integrations/claude-code/proxy-ensure.sh"
...
- [ ] **Step 4: Write `integrations/claude-code/proxy-ensure.sh`**
...
Run: `chmod +x integrations/claude-code/claude integrations/claude-code/proxy-ensure.sh && python -m pytest tests/test_claude_wrapper.py -v`
Expected: PASS, 6 tests.

- [ ] **Step 7: Lint the shell**

Run: `shellcheck integrations/claude-code/proxy-ensure.sh && shellcheck -s bash integrations/claude-code/claude`
Expected: no findings.
...
jobs:
  ...
  shell:
    ...
    steps:
      ...
      - run: shellcheck install.sh integrations/claude-code/proxy-ensure.sh
